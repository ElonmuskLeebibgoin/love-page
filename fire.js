...
function animate() {
    ...
    if (newTime - lastTime > 1000 + (window.innerHeight - 767) / 2) {
        var totalShapes = document.querySelectorAll(".shape").length;
        if (currentShapeIndex >= totalShapes) {
            allShapesShown = true;
        }
        
        var random = allShapesShown ? true : (Math.random() * 100 > 50 ? true : false);
        ...
        if (random) {
            ...
            var bigboom = new Boom(getRandom(canvas.width / 3, canvas.width * 2 / 3), 2, "#FFF", {
                ...
                document.querySelectorAll(".shape")[currentShapeIndex]);
            currentShapeIndex++;
            bigbooms.push(bigboom)
        }
        lastTime = newTime;
